...
def torch_sumAsin(output: torch.Tensor) -> torch.Tensor:
    total = None
    for i in range(0,output.shape[1],2):
        pair = output[:,i:i+1]*torch.sin(output[:,i+1:i+2])
        if total is None:
            total = pair
        else:
            total = total + pair
...
def torch_sumAsin_loss(output, target):
    loss = torch.mean((torch_sumAsin(output) - target) ** 2)
    return loss
